...
class CartClass extends Database{
	...
	$sql="SELECT MAX(invoice_id) AS invoice FROM tbl_order WHERE date='$datecheck'";
  ...
  public function addToCart($quantity,$id,$invoice,$userID){
      ...
      $sqlquery = "INSERT INTO tbl_cart(cartId,userId,foodNameId,foodName,price,quantity,image) VALUES(?,?,?,?,?,?,?)";
	  //$sqlquery = "INSERT INTO tbl_cart(foodNameId,quantity) VALUES(?,?)";
      $data = array($invoice,$userID,$id,$foodName,$price,$quantity,$image);
 ...
 public function submitTotal($user_id,$invoice){
	  ...
	  $sqlquery = "INSERT INTO tbl_order(invoice_id,user_id,food_id,food_name,food_image,food_price,food_quantity,date) VALUES(?,?,?,?,?,?,?,?)";
	  
      $dataa = array($cart_id,$user_id,$foodID,$foodName,$image,$price,$quantity,$datecheck);
